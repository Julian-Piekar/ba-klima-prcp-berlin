...
read_CER <- function(ncfpath, 
  ...
  CER_time <- seq(from = as.POSIXct(CER_time_start), length.out = dim(ncvar)[3], by = "hours")
  ...
  if (aggr[[1]] & aggr[[2]] == "sum"){
    ncvar_spm <- apply(ncvar, c(1,2),sum, na.rm=TRUE)
  }
...
proj_CER <- function(ncdata,ncdir, 
  ...
  fpath_2001 <- list.files(ncdir, full.names = T)[11]
